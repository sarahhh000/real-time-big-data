...
public class NGramMapper
    ...
    public void map(LongWritable key, Text value, Context context)
        throws IOException, InterruptedException{
            ...
            String tweet = line[4].replaceAll("https?://[^ ]*", "").trim();
            String blockNum = line[5];
            String[] words = tweet.replaceAll("[^a-zA-Z0-9 ]", "").toLowerCase().split("\\s+");
            ...
            for(int N = 2; N < 6; N++) {
                ...
                while(pos <= len - N) {
                    ...
                    if(flag == true && nwordstrim.length() > 0){
                        context.write(new Text(blockNum), new Text(nwordstrim));
                    }
                    pos = pos + 1;
                }
            }
